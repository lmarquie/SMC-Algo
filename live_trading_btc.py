...
class BTCLiveTradingBot:
    ...
    async def open_live_position(self, setup, current_price):
        """Open a live trading position on Hyperliquid for BTC"""
        ...
        try:
            ...
            self.logger.info(f"📈 Opening live position for BTC:")
            ...
            self.logger.info(f"📋 PLACING ORDER: BTC {'BUY' if is_buy else 'SELL'} {position_size} @ ${limit_price:.4f}")
            ...
            if order_result and 'status' in order_result and order_result['status'] == 'ok':
                ...
                if filled_data:
                    ...
                    self.logger.info(f"✅ LIVE POSITION OPENED FOR BTC:")
                    ...
                    send_telegram_message(
                        f" LIVE TRADE OPENED: BTC {setup['direction'].upper()} at ${actual_entry_price:.4f} | "
                    ...
                    send_telegram_message(f"⏳ BTC limit order placed at ${limit_price:.4f} - waiting 7 minutes for fill")
    ...
    def close_live_position(self, reason="manual"):
        ...
        try:
            self.logger.info(f"📉 Closing live position for BTC: {reason}")
            ...
            if close_result and 'status' in close_result and close_result['status'] == 'ok':
                ...
                if filled_data:
                    ...
                    self.logger.info(f"✅ LIVE POSITION CLOSED FOR BTC:")
                    ...
                    send_telegram_message(
                        f"{pnl_emoji} LIVE TRADE CLOSED: BTC at ${close_price:.4f} | "
        ...
        while self.stop_monitoring_active and self.current_position is not None:
            try:
                ...
                if direction == 'long' and current_price <= position['stop_loss']:
                    self.logger.info(f"🛑 CONTINUOUS MONITOR #{monitor_count}: STOP LOSS HIT for BTC LONG")
                    ...
                    self.logger.info(f"🛑 CONTINUOUS MONITOR #{monitor_count}: STOP LOSS HIT for BTC SHORT")
                    self.logger.info(f"  Price: ${current_price:.4f} >= Stop: ${position['stop_loss']:.4f}")
                    self.logger.info(f"  Final P&L: ${current_pnl:.4f} | R:R: {rr_ratio:.2f}")
                ...
                if position.get('take_profit') is not None:
                    if direction == 'long' and current_price >= position['take_profit']:
                        self.logger.info(f"🎯 CONTINUOUS MONITOR #{monitor_count}: TAKE PROFIT HIT for BTC LONG")
                        ...
                        self.logger.info(f"🎯 CONTINUOUS MONITOR #{monitor_count}: TAKE PROFIT HIT for BTC SHORT")
                        self.logger.info(f"  Price: ${current_price:.4f} <= Target: ${position['take_profit']:.4f}")
                        self.logger.info(f"  Final P&L: ${current_pnl:.4f} | R:R: {rr_ratio:.2f}")
                ...
                if self.current_position is not None:
                    ...
                    if initial_risk > 0:
                        ...
                        if monitor_count % 20 == 0:
                            self.logger.info(f"📊 RR MONITOR #{monitor_count}: BTC {direction.upper()}")
                        ...
                        if rr_ratio >= 3:
                            if position['direction'] == 'long':
                                ...
                                if position['stop_loss'] < new_stop:
                                    ...
                                    self.logger.info(f"🔄 RR MANAGEMENT #{monitor_count}: Moving BTC LONG stop to 1:1 RR")
                                    ...
                                    send_telegram_message(f"🔄 BTC LONG stop moved to 1:1 RR: ${old_stop:.4f} → ${new_stop:.4f} (RR: {rr_ratio:.2f})")
                            else:
                                new_stop = position['entry_price'] - initial_risk
                                if position['stop_loss'] > new_stop:
                                    old_stop = position['stop_loss']
                                    self.logger.info(f"🔄 RR MANAGEMENT #{monitor_count}: Moving BTC SHORT stop to 1:1 RR")
                                    self.logger.info(f"  Old Stop: ${old_stop:.4f} → New Stop: ${new_stop:.4f}")
                                    self.logger.info(f"  RR Ratio: {rr_ratio:.2f} | Profit: ${current_profit:.4f}")
                                    position['stop_loss'] = new_stop
                                    send_telegram_message(f"🔄 BTC SHORT stop moved to 1:1 RR: ${old_stop:.4f} → ${new_stop:.4f} (RR: {rr_ratio:.2f})")
            ...
            while True:  # Run indefinitely
                ...
                try:
                    ...
                    if self.pending_order:
                        ...
                        if order_placement_time:
                            ...
                            if time_since_order.total_seconds() > 420:  # 7 minutes = 420 seconds
                                ...
                                send_telegram_message(f"⏰ BTC order pending for 7+ minutes, cancelling...")
                                
                                try:
                                    self.client.cancel_order("BTC", self.pending_order['order_id'])
                                    self.logger.info(f"✅ Order cancelled successfully")
                                    send_telegram_message(f"✅ BTC order cancelled successfully")
                                except Exception as e:
                                    self.logger.error(f"❌ Failed to cancel order: {e}")
                                    send_telegram_message(f"❌ Failed to cancel BTC order: {e}")
                    ...
                    if pause_remaining and pause_remaining > 0:
                        self.logger.info(f"⏳ ORDER PAUSE ACTIVE for BTC: {pause_remaining:.0f} seconds remaining")
                        await asyncio.sleep(0.5)
                        continue
                    ...
                    if ltf_data is not None and current_price is not None:
                        ...
                        if self.current_position is not None:
                            ...
                            self.logger.info(f"🔄 TRAILING STOP CHECK: BTC {self.current_position['direction'].upper()}")
                            ...
                            if (self.current_position['direction'] == 'long' and self.current_position['stop_loss'] > old_stop_loss) or \
                               (self.current_position['direction'] == 'short' and self.current_position['stop_loss'] < old_stop_loss):
                                ...
                                self.logger.info(f"🔄 TRAILING STOP UPDATED for BTC {self.current_position['direction'].upper()}")
                                ...
                                send_telegram_message(f"🔄 TRAILING STOP UPDATED: BTC {self.current_position['direction'].upper()} ${old_stop_loss:.4f} → ${new_stop:.4f} (R:R: {rr_ratio:.2f})")
                            elif updated:
                                self.logger.debug(f"🔄 Trailing stop check completed - no update needed for BTC")
                            else:
                                self.logger.debug(f"🔄 Trailing stop check completed - no valid swing levels found for BTC")
