...
class RealDataBacktester:
    ...
    def _open_position(self, setup: Dict, current_price: float, timestamp):
        ...
        try:
            ...
            self.logger.info(f"Risk amount: ${risk_amount:.4f}, Position size: {position_size:.4f}, Expected loss if stopped: ${expected_loss:.2f}")
        ...
        for i, trade in enumerate(self.trades):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=[trade['entry_price']],
                    mode='markers+text',
                    marker=dict(color=color, size=12, symbol='triangle-up' if trade['direction'] == 'long' else 'triangle-down'),
                    text=[f'T{i+1} ENTRY'],
...
async def run_real_data_backtest():
    ...
    from config import SYMBOLS, HYPERLIQUID_API_KEY, HYPERLIQUID_SUBACCOUNT, TIMEFRAME, HTF_TIMEFRAME, POSITION_SIZE, BOS_LOOKBACK, DISPLACEMENT_THRESHOLD, STOP_LOSS_BUFFER, TAKE_PROFIT_RATIO, RISK_PER_TRADE, TRAILING_CONFIRMATION_CANDLES
    ...
    if all_trades:
        ...
        for symbol in SYMBOLS:
            ...
            if symbol_trades:
                ...
                print(f"  {symbol}: {len(symbol_trades)} trades, ${symbol_pnl:.2f} P&L, {symbol_win_rate:.1f}% win rate")
            else:
                print(f"  {symbol}: No trades")
    else:
