...
class ETHPaperTradingBot:
    ...
    def calculate_position_size(self, entry_price, stop_loss, direction):
        ...
        # ETH-specific: Ensure minimum $5 stop loss distance
        min_stop_distance = 5.0  # $5 minimum
        ...
        if direction == 'long':
            ...
            if current_stop_distance < min_stop_distance:
                ...
                self.logger.info(f"ETH: Stop loss adjusted to minimum $5 distance: ${stop_loss:.4f}")
        else:  # short
        ...
        leverage = self.config['MAX_LEVERAGE'].get("ETH", 20)  # Default to 20x for ETH
    ...
    def open_paper_position(self, setup, current_price):
        ...
        try:
            ...
            # Get leverage for ETH
            leverage = self.config['MAX_LEVERAGE'].get("ETH", 20)
            ...
            send_telegram_message(
                f"Trade OPENED: ETH {setup['direction'].upper()} at ${setup['entry_price']:.2f} | Stop: ${final_stop:.2f} | Leverage: {leverage}x"
            )
    ...
    def close_paper_position(self, current_price, reason):
        ...
        try:
            ...
            self.logger.info(f"⏳ COOLDOWN STARTED: 5-minute cooldown after position close for ETH")
                    ...
                    if ltf_data is not None and current_price is not None:
                        ...
                        if self.current_position is None:
                            ...
                            if setup:
                                ...
                                self.logger.info(f"🎯 TRADE SETUP DETECTED for ETH: {setup['direction'].upper()} at ${setup['entry_price']:.2f}")
                                send_telegram_message(f"🎯 TRADE SETUP: ETH {setup['direction'].upper()} at ${setup['entry_price']:.2f}")
