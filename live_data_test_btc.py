...
class BTCPaperTradingBot:
    ...
    def calculate_position_size(self, entry_price, stop_loss, direction):
        ...
        leverage = self.config['MAX_LEVERAGE'].get("BTC", 25)  # Default to 25x for BTC
    ...
    def open_paper_position(self, setup, current_price):
        """Open a paper trading position for BTC"""
        if self.current_position is not None:
            self.logger.warning(f"Already in a position for BTC, cannot open new one")
            return False
        
        try:
            self.logger.info(f"DEBUG: Opening position for BTC with setup: {setup}")
            ...
            # Get leverage for BTC
            leverage = self.config['MAX_LEVERAGE'].get("BTC", 25)
            ...
            self.logger.info(f"DEBUG: Position created for BTC")
            
            self.logger.info(f"📈 PAPER POSITION OPENED FOR BTC:")
            ...
            self.logger.info(f"DEBUG: About to send Telegram notification for BTC")
            send_telegram_message(
                f"Trade OPENED: BTC {setup['direction'].upper()} at ${setup['entry_price']:.4f} | Stop: ${final_stop:.4f} | Leverage: {leverage}x"
            )
            self.logger.info(f"DEBUG: Telegram notification sent for BTC")
    ...
    def close_paper_position(self, current_price, reason):
        """Close the current paper trading position for BTC"""
        if self.current_position is None:
            self.logger.warning(f"❌ ATTEMPTED TO CLOSE NON-EXISTENT POSITION for BTC")
            return
        ...
        try:
            ...
            self.logger.info(f"🚨 BALANCE UPDATE for BTC: ${old_balance:.2f} → ${self.paper_balance:.2f} (+${pnl_dollar:.2f})")
            self.logger.info(f"🚨 TOTAL P&L UPDATE for BTC: ${old_total_pnl:.2f} → ${self.total_pnl:.2f} (+${pnl_dollar:.2f})")
            ...
            self.logger.info(f"DEBUG: Balance updated for BTC: P&L ${pnl_dollar:.2f}, New balance: ${self.paper_balance:.2f}, Total trades: {self.total_trades}, Winning trades: {self.winning_trades}")
            self.logger.info(f"DEBUG: Trade history now has {len(self.trade_history)} trades")
            
            # Log the trade
            self.logger.info(f"📉 PAPER POSITION CLOSED FOR BTC:")
            ...
            self.logger.info(f"DEBUG: Position deleted for BTC")
            
            # Calculate win rate
            win_rate = (self.winning_trades / self.total_trades * 100) if self.total_trades > 0 else 0
            
            send_telegram_message(
                f"Trade CLOSED: BTC {position['direction'].upper()} | Entry: ${position['entry_price']:.4f} | Exit: ${current_price:.4f} | Size: {position['size']:.4f} | P&L: ${pnl_dollar:.2f}"
            )
    ...
    def check_position_exits(self, current_price, current_low, current_high, candle_idx, ltf_data, cycle_count):
        """Check if current position should be closed for BTC"""
        if self.current_position is None:
            return
        ...
        if cycle_count % 5 == 0:  # Show every 5 cycles
            self.logger.info(f"🔍 CHECKING TRAILING STOPS for BTC - Current stop: ${old_stop_loss:.4f}")
        ...
        if position['stop_loss'] != old_stop_loss:
            self.logger.info(f"🔄 TRAILING STOP UPDATED for BTC: ${old_stop_loss:.4f} → ${position['stop_loss']:.4f}")
            send_telegram_message(f"🔄 TRAILING STOP UPDATED: BTC ${old_stop_loss:.4f} → ${position['stop_loss']:.4f}")
        else:
            # Show current stop status even when not changed
            if cycle_count % 10 == 0:  # Show every 10 cycles to avoid spam
                self.logger.info(f"📊 BTC STOP STATUS: Current stop: ${position['stop_loss']:.4f} (unchanged)")
        
        # Check if stop loss is hit
        if direction == 'long' and current_low <= position['stop_loss']:
            self.logger.info(f"🛑 STOP LOSS HIT for BTC LONG: Low ${current_low:.4f} <= Stop ${position['stop_loss']:.4f}")
            send_telegram_message(f"🛑 STOP LOSS HIT: BTC LONG at ${position['stop_loss']:.4f}")
            ...
            self.logger.info(f"🛑 STOP LOSS HIT for BTC SHORT: High ${current_high:.4f} >= Stop ${position['stop_loss']:.4f}")
            send_telegram_message(f"🛑 STOP LOSS HIT: BTC SHORT at ${position['stop_loss']:.4f}")
            self.close_paper_position(position['stop_loss'], "Stop Loss Hit")
            self.last_stop_idx = candle_idx
    
    def print_paper_trading_summary(self, current_price, trend_info, setup):
        """Print comprehensive paper trading summary for BTC"""
        print(f"\n📊 BTC PAPER TRADING SUMMARY")
        print("="*50)
    ...
    async def run_paper_trading(self, duration_minutes=None):
        ...
        if duration_minutes:
            self.logger.info(f"🚀 Starting BTC paper trading for {duration_minutes} minutes...")
            start_time = datetime.now()
            end_time = start_time + timedelta(minutes=duration_minutes)
        else:
            self.logger.info(f"🚀 Starting BTC paper trading INDEFINITELY...")
        ...
        send_telegram_message(
            f"🚀 BTC BOT STARTED: Balance: ${self.paper_balance:.2f} | Risk: ${self.config['RISK_PER_TRADE']}"
        )
        
        # Test notification
        send_telegram_message("🧪 TEST: BTC notification system is working")
        ...
        try:
            while True:  # Run indefinitely
                ...
                if cycle_count % 20 == 0:  # Log every 20 cycles (about 10 minutes)
                    self.logger.info(f"🔄 BTC paper trading cycle {cycle_count} - Balance: ${self.paper_balance:.2f}, Total P&L: ${self.total_pnl:.2f}")
                    ...
                    if self.total_trades > 0:
                        win_rate = (self.winning_trades / self.total_trades * 100)
                        send_telegram_message(
                            f"📊 BTC PERIODIC UPDATE: Balance: ${self.paper_balance:.2f} | P&L: ${self.total_pnl:.2f} | Trades: {self.total_trades} | Win Rate: {win_rate:.1f}%"
                        )
                    ...
                    if ltf_data is not None and current_price is not None:
                        ...
                        if self.current_position is not None:
                            ...
                            if (self.current_position['direction'] == 'long' and self.current_position['stop_loss'] > old_stop_loss) or \
                               (self.current_position['direction'] == 'short' and self.current_position['stop_loss'] < old_stop_loss):
                                self.logger.info(f"🔄 TRAILING STOP UPDATED for BTC: ${old_stop_loss:.4f} → ${self.current_position['stop_loss']:.4f}")
                                send_telegram_message(f"🔄 TRAILING STOP UPDATED: BTC ${old_stop_loss:.4f} → ${self.current_position['stop_loss']:.4f}")
                        ...
                        if self.current_position is None:
                            ...
                            if setup and (candle_idx - self.last_stop_idx >= 5):  # Cooldown check
                                ...
                                self.logger.info(f"🎯 TRADE SETUP DETECTED for BTC: {setup['direction'].upper()} at ${setup['entry_price']:.4f}")
                                send_telegram_message(f"🎯 TRADE SETUP: BTC {setup['direction'].upper()} at ${setup['entry_price']:.4f}")
                                ...
                                self.logger.info(f"⏳ SETUP DETECTED but in cooldown for BTC (cooldown: {candle_idx - self.last_stop_idx}/5)")
                        else:
                            self.logger.info(f"📊 BTC already has position: {self.current_position['direction']} at ${self.current_position['entry_price']:.4f}")
                        
                        # Print summary for BTC (less frequently to avoid spam)
            ...
            self.logger.info("🛑 BTC paper trading stopped by user (Ctrl+C)")
    ...
    def print_final_summary(self):
        ...
        print("\n" + "="*70)
        print("🏁 FINAL BTC PAPER TRADING SUMMARY")
        print("="*70)
    ...
    async def monitor_stops_continuously(self):
        ...
        self.logger.info("🔍 Starting continuous stop monitoring for BTC")
        ...
        while self.stop_monitoring_active and self.current_position is not None:
            try:
                ...
                if direction == 'long' and current_price <= position['stop_loss']:
                    self.logger.info(f"🛑 CONTINUOUS MONITOR: STOP LOSS HIT for BTC LONG: Price ${current_price:.4f} <= Stop ${position['stop_loss']:.4f}")
                    send_telegram_message(f"🛑 CONTINUOUS MONITOR: BTC LONG STOP HIT at ${position['stop_loss']:.4f}")
                    ...
                    self.logger.info(f"🛑 CONTINUOUS MONITOR: STOP LOSS HIT for BTC SHORT: Price ${current_price:.4f} >= Stop ${position['stop_loss']:.4f}")
                    send_telegram_message(f"🛑 CONTINUOUS MONITOR: BTC SHORT STOP HIT at ${position['stop_loss']:.4f}")
                    self.close_paper_position(position['stop_loss'], "Stop Loss Hit (Continuous Monitor)")
                    self.stop_monitoring_active = False
                    break
                ...
                if position.get('take_profit') is not None:
                    if direction == 'long' and current_price >= position['take_profit']:
                        self.logger.info(f"🎯 CONTINUOUS MONITOR: TAKE PROFIT HIT for BTC LONG: Price ${current_price:.4f} >= Target ${position['take_profit']:.4f}")
                        send_telegram_message(f"🎯 CONTINUOUS MONITOR: BTC LONG TAKE PROFIT at ${position['take_profit']:.4f}")
                        ...
                        self.logger.info(f"🎯 CONTINUOUS MONITOR: TAKE PROFIT HIT for BTC SHORT: Price ${current_price:.4f} <= Target ${position['take_profit']:.4f}")
                        send_telegram_message(f"🎯 CONTINUOUS MONITOR: BTC SHORT TAKE PROFIT at ${position['take_profit']:.4f}")
                        self.close_paper_position(position['take_profit'], "Take Profit Hit (Continuous Monitor)")
                        self.stop_monitoring_active = False
                        break
...
async def main():
    """Main function to run BTC paper trading"""
    bot = BTCPaperTradingBot()
...
if __name__ == "__main__":
    print("🚀 BTC Paper Trading Bot - No Real Money")
    print("This bot simulates trading BTC with virtual money.")
