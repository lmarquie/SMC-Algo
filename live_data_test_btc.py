...
class BTCPaperTradingBot:
    ...
    def calculate_position_size(self, entry_price, stop_loss, direction):
        ...
        # BTC-specific: Ensure minimum $100 stop loss distance
        min_stop_distance = 100.0  # $100 minimum
        ...
        if direction == 'long':
            ...
            if current_stop_distance < min_stop_distance:
                ...
                self.logger.info(f"BTC: Stop loss adjusted to minimum $100 distance: ${stop_loss:.4f}")
        else:  # short
        ...
        leverage = self.config['MAX_LEVERAGE'].get("BTC", 20)  # Default to 20x for BTC
    ...
    def open_paper_position(self, setup, current_price):
        ...
        try:
            ...
            # Get leverage for BTC
            leverage = self.config['MAX_LEVERAGE'].get("BTC", 20)
            ...
            send_telegram_message(
                f"Trade OPENED: BTC {setup['direction'].upper()} at ${setup['entry_price']:.2f} | Stop: ${final_stop:.2f} | Leverage: {leverage}x"
            )
    ...
    def close_paper_position(self, current_price, reason):
        ...
        try:
            ...
            self.logger.info(f"⏳ COOLDOWN STARTED: 5-minute cooldown after position close for BTC")
                    ...
                    if ltf_data is not None and current_price is not None:
                        ...
                        if self.current_position is None:
                            ...
                            if cooldown_remaining and cooldown_remaining > 0:
                                self.logger.info(f"⏳ COOLDOWN ACTIVE for BTC: {cooldown_remaining:.0f} seconds remaining")
                                candle_idx += 1
                            ...
                            if setup:
                                ...
                                self.logger.info(f"🎯 TRADE SETUP DETECTED for BTC: {setup['direction'].upper()} at ${setup['entry_price']:.2f}")
                                send_telegram_message(f"🎯 TRADE SETUP: BTC {setup['direction'].upper()} at ${setup['entry_price']:.2f}")
                            ...
                            self.logger.info(f"📊 BTC already has position: {self.current_position['direction']} at ${self.current_position['entry_price']:.2f}")
