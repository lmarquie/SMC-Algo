...
class SOLLiveTradingBot:
    ...
    def calculate_position_size(self, entry_price, stop_loss, direction):
        ...
        if direction == 'long':
            ...
            if current_stop_distance < min_stop_distance:
                ...
                self.logger.info(f"SOL: Strategy stop loss adjusted to minimum 0.15% distance: ${stop_loss:.4f} ({min_stop_distance*100:.4f}%)")
        else:  # short
        ...
        leverage = self.config['MAX_LEVERAGE'].get("SOL", 20)  # Use 20x like backtest
    ...
    def process_elixir_update(self, state_data):
        ...
        try:
            ...
            if fills and self.pending_order:
                ...
                if filled_this_update > 0:
                    ...
                    send_telegram_message(
                        f"✅ PARTIAL FILL: SOL {self.pending_order['direction'].upper()} "
            ...
            if positions and self.pending_order:
                # Only check SOL positions
                SOL_positions = [pos for pos in positions if pos.get('coin') == 'SOL']
                if SOL_positions:
                    self.logger.info(f"🔍 Checking {len(SOL_positions)} SOL positions for our pending order")
                    
                    for position in SOL_positions:
                        position_size = float(position.get('size', 0))
                        
                        if position_size != 0:
                            self.logger.info(f"🔍 Found SOL position: size {position_size}")
                            ...
                            send_telegram_message(
                                f"✅ MOMENTUM ALO FILLED: SOL {self.pending_order['direction'].upper()} at ${entry_price:.4f} | "
    ...
    async def monitor_pending_order(self):
        ...
        while (datetime.now() - start_time).total_seconds() < timeout_seconds:
            try:
                ...
                if status['status'] in ['found', 'resting_via_query']:
                    ...
                    send_telegram_message(
                        f"✅ MOMENTUM ALO FILLED: SOL {direction.upper()} at ${self.current_position['entry_price']:.4f} | "
                    ...
                    # Send Telegram notification IMMEDIATELY
                    send_telegram_message(
                        f"✅ ALO ORDER FILLED: SOL {direction.upper()} at ${self.current_position['entry_price']:.4f} | "
        ...
        if cancel_result['status'] == 'cancelled':
            ...
            send_telegram_message(f"⏰ ALO ORDER CANCELLED: SOL {direction.upper()} (5min timeout)")
        else:
    ...
    async def open_live_position(self, setup, current_price):
        ...
        try:
            ...
            self.logger.info(f"📈 Opening position for SOL (ALO LIMIT ORDER):")
            ...
            order_result = await self.place_limit_order("SOL", is_buy, position_size, limit_price, "Alo")
            ...
            if order_result['status'] == 'filled':
                ...
                # Send Telegram notification
                send_telegram_message(
                    f"✅ ALO FILLED: SOL {setup['direction'].upper()} at ${order_result['fill_price']:.4f} | "
                ...
                # Send Telegram notification
                send_telegram_message(
                    f"⏳ ALO PLACED: SOL {setup['direction'].upper()} @ ${limit_price:.4f} | "
    ...
    async def close_live_position(self, reason="manual"):
        ...
        try:
            self.initialize_api_clients()
            self.logger.info(f"📉 Closing live position for SOL (GTC LIMIT ORDER): {reason}")
            ...
            if order_result and 'status' in order_result:
                ...
                if order_result['status'] == 'filled':
                    ...
                    self.logger.info(f"✅ LIVE POSITION CLOSED FOR SOL (GTC IMMEDIATE FILL):")
                    ...
                    send_telegram_message(
                        f"{pnl_emoji} GTC IMMEDIATE CLOSE: SOL at ${close_price:.4f} | "
                    ...
                    while elapsed_time < max_wait_time:
                        try:
                            # Check order status
                            status_result = await self.check_order_status_by_cloid("SOL", cloid)
                            ...
                            if status_result and 'status' in status_result:
                                if status_result['status'] == 'filled_via_position':
                                    ...
                                    self.logger.info(f"✅ LIVE POSITION CLOSED FOR SOL (GTC FILL):")
                                    self.logger.info(f"  Exit Price: ${close_price:.4f}")
                                    self.logger.info(f"  Raw P&L: ${pnl + exit_fee:.2f}")
                                    self.logger.info(f"  Exit Fee: ${exit_fee:.2f}")
                                    self.logger.info(f"  Net P&L: ${pnl:.2f}")
                                    ...
                                    pnl_emoji = "🟢" if pnl > 0 else "🔴"
                                    send_telegram_message(
                                        f"{pnl_emoji} GTC TRADE CLOSED: SOL at ${close_price:.4f} | "
                                        f"Net P&L: ${pnl:.2f} | Fees: ${trade_record['total_fees']:.2f} | Reason: {reason} | Total: ${self.total_pnl:.2f}"
                                    )
        ...
        try:
            ...
            if close_result and 'status' in close_result and close_result['status'] == 'ok':
                ...
                if filled_data:
                    ...
                    self.logger.info(f"✅ LIVE POSITION CLOSED FOR SOL (MARKET FALLBACK):")
                    self.logger.info(f"  Exit Price: ${close_price:.4f}")
                    ...
                    pnl_emoji = "🟢" if pnl > 0 else "🔴"
                    send_telegram_message(
                        f"{pnl_emoji} MARKET FALLBACK CLOSED: SOL at ${close_price:.4f} | "
    ...
    def start_stop_monitoring(self):
        ...
        if self.current_position is not None:
            ...
            if (
                ...
                self.logger.info("FORCE STARTING continuous stop monitoring for SOL")
    ...
    async def monitor_stops_continuously(self):
        ...
        self.logger.info("🔍 Starting FVG-based momentum stop monitoring for SOL")
            ...
            try:
                ...
                if position.get('strategy_type') == 'momentum':
                    ...
                    if 'trailing_enabled' not in position:
                        ...
                        send_telegram_message(f"✅ MOMENTUM TRAILING ENABLED for SOL")
                        ...
                        try:
                            ...
                            if ltf_data is not None:
                                ...
                                if updated:
                                    ...
                                    send_telegram_message(f"🔄 MOMENTUM TRAILING: SOL ${old_stop:.4f} → ${new_stop:.4f} (R:R: {rr_ratio:.2f})")
                        except Exception as e:
                            self.logger.error(f"Error updating momentum trailing stop: {e}")
            
                # Check if stop loss is hit
                if direction == 'long' and current_price <= position['stop_loss']:
                    self.logger.info(f"🛑 MOMENTUM MONITOR #{monitor_count}: FVG STOP HIT for SOL LONG")
                    ...
                    send_telegram_message(f"🛑 FVG STOP HIT: SOL LONG at ${position['stop_loss']:.4f}")
                    await self.close_live_position("FVG Stop Loss Hit")
                    self.stop_monitoring_active = False
                    break
                elif direction == 'short' and current_price >= position['stop_loss']:
                    self.logger.info(f"🛑 MOMENTUM MONITOR #{monitor_count}: FVG STOP HIT for SOL SHORT")
                    self.logger.info(f"  Price: ${current_price:.4f} >= FVG Stop: ${position['stop_loss']:.4f}")
                    self.logger.info(f"  Final P&L: ${current_pnl:.4f} | R:R: {rr_ratio:.2f}")
                    send_telegram_message(f"🛑 FVG STOP HIT: SOL SHORT at ${position['stop_loss']:.4f}")
                    await self.close_live_position("FVG Stop Loss Hit")
                    self.stop_monitoring_active = False
                    break
            ...
            while True:  # Run indefinitely
                ...
                try:
                    ...
                    if cycle_count % 20 == 0:  # Every 10 seconds (20 cycles * 0.5s)
                        self.logger.info(f"🔄 TRADING CYCLE #{cycle_count} - Searching for SOL setups...")
                    ...
                    if self.current_position is not None or self.pending_order is not None:
                        ...
                        if self.current_position:
                            self.logger.info(f"📊 SOL already has active position: {self.current_position['direction']} at ${self.current_position['entry_price']:.2f}")
                        if self.pending_order:
                            self.logger.info(f"⏳ SOL has pending ALO order: {self.pending_order['direction']} @ ${self.pending_order['limit_price']:.2f}")
                    ...
                    if cooldown_remaining and cooldown_remaining > 0:
                        self.logger.info(f"⏳ COOLDOWN ACTIVE for SOL: {cooldown_remaining:.0f}s remaining")
                        candle_idx += 1
                        await asyncio.sleep(0.5)
                        continue
                    ...
                    if cycle_count % 10 == 0:  # Log every 10 cycles when searching
                        self.logger.info(f"🔍 MAIN LOOP: Searching for SOL trade setups... (Cycle #{cycle_count})")
        ...
        try:
            ...
            if user_state and 'assetPositions' in user_state:
                for asset_pos in user_state['assetPositions']:
                    if asset_pos.get('position', {}).get('coin') == 'SOL':
                        position_data = asset_pos.get('position', {})
                        size = float(position_data.get('szi', 0))
                        
                        if size != 0:  # Position has size
                            self.logger.info(f"🔍 FOUND EXISTING SOL POSITION: {position_data}")
