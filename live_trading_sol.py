...
class SOLLiveTradingBot:
    ...
    def calculate_position_size(self, entry_price, stop_loss, direction):
        ...
        min_stop_distance = 0.25  # 3 cents minimum
                    ...
                    for fill in recent_fills:
                        ...
                        if fill_coin == 'SOL' and fill_oid:
                            ...
                            if (pending_regular_id_int == fill_oid_int or 
                                ...
                                self.logger.info(f"🔍 POTENTIAL MATCH: SOL fill OID {fill_oid}")
                                ...
                                send_telegram_message(
                                    f"✅ MOMENTUM ALO FILLED: SOL {self.pending_order['direction'].upper()} at ${fill_price:.4f} | "
    ...
    async def open_live_position(self, setup, current_price):
        ...
        try:
            ...
            self.logger.info(f"📈 Opening MOMENTUM position for SOL (INVERTED ALO LIMIT ORDER):")
            ...
            if order_result['status'] == 'filled':
                ...
                # Send Telegram notification
                send_telegram_message(
                    f"✅ MOMENTUM ALO FILLED: SOL {setup['direction'].upper()} at ${order_result['fill_price']:.4f} | "
                ...
                # Send Telegram notification
                send_telegram_message(
                    f"⏳ MOMENTUM ALO PLACED: SOL {setup['direction'].upper()} @ ${limit_price:.4f} | "
    ...
    def close_live_position(self, reason="manual"):
        ...
        try:
            self.initialize_api_clients()
            self.logger.info(f"📉 Closing live position for SOL (INSTANT EXECUTION): {reason}")
            ...
            if close_result and 'status' in close_result and close_result['status'] == 'ok':
                ...
                if filled_data:
                    ...
                    self.logger.info(f"✅ LIVE POSITION CLOSED FOR SOL (INSTANT EXECUTION):")
                    ...
                    send_telegram_message(
                        f"{pnl_emoji} INSTANT TRADE CLOSED: SOL at ${close_price:.4f} | "
